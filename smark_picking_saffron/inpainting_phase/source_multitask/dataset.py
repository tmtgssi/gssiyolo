...
class get_dataset(Dataset):
    def __init__(self, pt_dataset, mask_path=None, test_mask_path=None, is_train=False, image_size=256):
        ...
        if is_train:    #train的的部分 (train_dataset)
            ...
            self.mask_list = []   #清空
            # print(self.mask_list)
            with open(test_mask_path) as f:   #開第一個mask_path
                for line in f:
                    self.mask_list.append(line.strip())   #把裡面的圖片一個一個加進去
    ...
    def load_mask(self, img, index):
        ...
        if self.training is False:  #test的部分
            # print(len(self.mask_list),index)
            ...
            mask = cv2.imread(self.strokes_mask_list[index],
            ...
            return mask
    # def load_mask(self, img, index):
    #     imgh, imgw = img.shape[0:2]  # only take height and width
        
    #     if self.training is False:
    #         # Test mode: load predefined mask
    ...
    def load_edge(self, img):
        return canny(img, sigma=2, mask=None).astype(float)  #轉邊緣圖
