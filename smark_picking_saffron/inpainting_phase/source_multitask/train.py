...
from losses.CEL import cel_loss
...
# Get the tensor by image path or name
import numpy as np

def get_tensor_by_name(image_paths_or_names, csv_df):
    """
    image_paths_or_names: str or list of str
    csv_df: pandas DataFrame
    Returns: Tensor of shape (B, N), where B is batch size, N is vector dimension
    """
    if isinstance(image_paths_or_names, str):
        image_paths_or_names = [image_paths_or_names]  # convert to list
    
    vectors = []
    
    for name in image_paths_or_names:
        row = csv_df[csv_df[0].astype(str).apply(lambda path: name in path)]
        if row.empty:
            raise ValueError(f"No entry found for '{name}' in CSV.")
        vector = row.iloc[0, 1:].values.astype('float32')  # shape (N,)
        vectors.append(vector)
    
    batch_tensor = torch.tensor(np.stack(vectors))  # shape (B, N)
    return batch_tensor
...
# Load the CSV
df = pd.read_csv(args['ssi_file'], header=None)
# set log
...
for epoch in range(args['train_epoch']):
    ...
    for it, items in enumerate(tqdm(loader, disable=False)):
        ...
        HSV_loss = HSV(img, pred_img, Edge)
        # print(f"ssi_gt device: {ssi_gt.device}")
        # print(f"ssi_pred device: {ssi_pred.device}")
        SSI_loss= cel_loss(ssi_gt,ssi_pred)
        ...
        if iterations % 50 == 0:
            ...
            writer.add_scalar('loss/SSI', SSI_loss.item(), iterations)
    ...
    for val_it, val_items in enumerate(tqdm(loader1, disable=True)):
        for k in val_items:
        ...
        with torch.no_grad():
            ...
            pred_img,_ = raw_model(val_items['img'], val_items['mask'])
